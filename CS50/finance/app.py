...
def index():
    ...
    database = db.execute(
        "SELECT symbol, name, price, SUM(shares) AS total_S FROM transactions WHERE user_id = ? GROUP BY symbol", user_id)
    cash_db = db.execute("SELECT cash FROM users WHERE id = ?", user_id)
    cash = cash_db[0]["cash"]
    total = cash
    for transaction in database:
        total += transaction["price"]*transaction["total_S"]

    return render_template("index.html", database=database, cash=cash, usd=usd, total=total)
...
def buy():
    ...
    if request.method == "GET":
        ...
        try:
            ...
            return apology(" shares not be integer ")
        ...
        db.execute("INSERT INTO transactions (user_id, name, shares, price, type, symbol) VALUES (?, ?, ?, ?, ?, ?)",
                   user_id, stock["name"],  shares, stock["price"], "buy",  stock["symbol"])
...
def sell():
    ...
    if request.method == "GET":
        ...
        if not stock:
            return apology("symbol noy exist")
        ...
        if shares > user_shares_real:
            return apology(" you dont have this amount or shares")

        uptd_cash = user_cash + transaction_value
        db.execute("UPDATE users SET cash = ? WHERE id = ?", uptd_cash, user_id)
        db.execute("INSERT INTO transactions (user_id, name, shares, price, type, symbol) VALUES (?, ?, ?, ?, ?, ?)",
                   user_id, stock["name"],  -shares, stock["price"], "sell",  stock["symbol"])
