...
def train_model(evidence, labels):
    ...
    """
    # Given a list of evidence lists and a list of labels
    model = KNeighborsClassifier(n_neighbors=1)
    model.fit(evidence, labels)

    # return a fitted k-nearest neighbor model
    return model
...
def evaluate(labels, predictions):
    ...
    """
    # l_pos = posative , l_neg = negative
    l_pos = labels.count(1)
    l_neg = labels.count(0)

    # pos = posative , neg = negative
    pos = 0
    neg = 0
    ...
    sensitivity = pos / l_pos
    specificity = neg / l_neg
