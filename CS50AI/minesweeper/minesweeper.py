    ...
    def add_knowledge(self, cell, count):
        ...
        for sentance in self.knowledge:
            if sentance.known_mines():
                for cell in sentance.known_mines().copy():
                    self.mark_mine(cell)
            if sentance.known_safes():
                for cell in sentance.known_safes().copy():
                    self.mark_safe(cell)

        for sentance in self.knowledge:
            if new_sentence.cells.issubset(sentance.cells):
                if sentance.count > 0 and new_sentence.count > 0:
                    if new_sentence != sentance:
                        new_subset = sentance.cells.difference(new_sentence.cells)
                        new_sentence_subset = Sentence(list(new_subset), sentance.count - new_sentence.count)
