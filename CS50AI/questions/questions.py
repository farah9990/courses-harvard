...
def compute_idfs(documents):
    ...
    word_idfs = dict()
    for word in counts:
        word_idfs[word] = math.log((num_d / counts[word]))
...
def top_files(query, files, idfs, n):
    ...
    stored_f = sorted(file_scores.items(), key=lambda x: x[1], reverse=True)
    stored_f = [x[0] for x in stored_f]
